        ...
        <div class="collapse navbar-collapse" id="navbarNav">
            ...
            <div class="navbar-nav me-auto">
                <a class="nav-link nav-item {{ request()->routeIs('home') ? 'active' : '' }}"
                    href="{{ route('home') }}">
                    <i class="fas fa-home me-1"></i>{{ __('main.home') }}
                </a>
                <a class="nav-link nav-item {{ request()->routeIs('products.*') ? 'active' : '' }}"
                    href="{{ route('products.index') }}">
                    <i class="fas fa-laptop me-1"></i>{{ __('main.products') }}
                </a>
                <a class="nav-link nav-item {{ request()->routeIs('about') ? 'active' : '' }}"
                    href="{{ route('about.index') }}">
                    <i class="fas fa-info-circle me-1"></i>{{ __('main.about') }}
                </a>
                <a class="nav-link nav-item {{ request()->routeIs('contact') ? 'active' : '' }}"
                    href="{{ route('contact.index') }}">
                    <i class="fas fa-envelope me-1"></i>{{ __('main.contact') }}
                </a>

                <!-- Marketplace егер орындалса -->
                @if(config('features.marketplace'))
                <a class="nav-link nav-item {{ request()->routeIs('marketplace.*') ? 'active' : '' }}"
                    href="{{ route('marketplace.index') }}">
                    <i class="fas fa-store me-1"></i>{{ __('main.marketplace') }}
                </a>
                @endif
            </div>
            ...
            <div class="navbar-nav mx-4">
                ...
                <a class="nav-link nav-item {{ request()->routeIs('cart') ? 'active' : '' }}"
                    ...
                    <i class="fas fa-shopping-cart me-1"></i>{{ __('main.cart') }}
                </a>

                <a class="nav-link nav-item {{ request()->routeIs('profile') ? 'active' : '' }}"
                    href="{{ route('profile.index') }}">
                    <i class="fas fa-user me-1"></i>{{ __('main.profile') }}
                </a>
                ...
                <form method="POST" action="{{ route('logout') }}" class="d-inline">
                    ...
                    <a class="nav-link nav-item logout-link"
                        ...
                        <i class="fas fa-sign-out-alt me-1"></i>{{ __('main.logout') }}
                    </a>
                </form>
                ...
                <a class="nav-link nav-item {{ request()->routeIs('login') ? 'active' : '' }}"
                    href="{{ route('login') }}">
                    <i class="fas fa-sign-in-alt me-1"></i>{{ __('main.login') }}
                </a>
                <a class="nav-link nav-item register-link {{ request()->routeIs('register') ? 'active' : '' }}"
                    href="{{ route('register') }}">
                    <i class="fas fa-user-plus me-1"></i>{{ __('main.register') }}
                </a>
                @endauth
            </div>
            ...
            <div class="navbar-nav ms-auto align-items-center">
                ...
                <a class="nav-link nav-item admin-link {{ request()->routeIs('admin.*') ? 'active' : '' }}"
                    ...
                    <i class="fas fa-crown me-1"></i>{{ __('main.admin') }}
                </a>
                ...
                <div class="language-selector">
                    <a href="{{ request()->fullUrlWithQuery(['lang' => 'kaz']) }}"
                        class="lang-btn {{ app()->getLocale() == 'kaz' ? 'active' : '' }}">
                        ҚАЗ
                    </a>
                    <a href="{{ request()->fullUrlWithQuery(['lang' => 'rus']) }}"
                        class="lang-btn {{ app()->getLocale() == 'rus' ? 'active' : '' }}">
                        РУС
                    </a>
